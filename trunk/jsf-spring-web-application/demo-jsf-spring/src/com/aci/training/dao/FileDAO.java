...
public class FileDAO {
	...
	public File addFile(File file) {
		...
		/*
		 * Blob content = (Blob) jdbcTemplate.queryForObject(
		 * "SELECT content FROM files WHERE file_name='"+file.getFileName()+"'",
		 * Blob.class); // try { // OutputStream s = content.setBinaryStream(0);
		 * // } catch (SQLException e) { // // TODO Auto-generated catch block
		 * // e.printStackTrace(); // }
		 * 
		 * if (content==null){ System.out.println("Asdfsadfasdf"); } try { if
		 * (file.getContent().length != content.setBytes(0, file .getContent()))
		 * { return null; } } catch (SQLException e) { // TODO Auto-generated
